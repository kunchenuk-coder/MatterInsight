...
interface PinterestFeedProps {
  materials: Material[];
  publishedMoodboards?: MoodBoard[];
  onSelect: (material: Material) => void;
  onSelectMoodboard?: (board: MoodBoard) => void;
  onSave: (id: string, moodboardId?: string, newMoodboardName?: string) => void;
  savedIds: string[];
  moodboards?: MoodBoard[];
  collectedMoodboardIds?: string[];
  onToggleCollectMoodboard?: (moodboardId: string) => void;
}

const PinterestFeed: React.FC<PinterestFeedProps> = ({
  materials,
  publishedMoodboards = [],
  onSelect,
  onSelectMoodboard,
  ...
  const feed = buildMixedFeed(materials, publishedMoodboards);
      ...
      {feed.map((entry) => {
        if (entry.kind === 'moodboard') {
          ...
          return (
            <div
              key={`mb-${board.id}`}
              onClick={() => onSelectMoodboard?.(board)}
            ...
            >
              {cover ? (
                ...
                <div className="w-full aspect-[4/3] bg-gray-100 flex items-center justify-center text-gray-300 text-4xl">
                  🎨
                </div>
              )}
              <div className="p-4">
                <div className="flex justify-between items-start mb-1 gap-2">
                  <h3 className="font-bold text-gray-900 line-clamp-1">{board.name}</h3>
                  <span className="text-[9px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-black uppercase shrink-0">
                    Published
                  </span>
                </div>
                {board.ownerId ? (
                  ...
                  <p className="text-xs text-gray-500 mb-1 font-bold">{ownerName}</p>
                )}
                    ...
                    <div className="absolute bottom-full right-0 mb-2 w-48 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden">
                      ...
                      {isCreatingNewFromFeed === mat.id ? (
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            setIsCreatingNewFromFeed(mat.id);
                          }}
